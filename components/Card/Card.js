...
*/}
    <div>
        <div className={g.div1}>
            <input type='text' placeholder='Enter Widget Name' className={g.searchInput}/>
            <div className={g.buttonGroup}>
             <button className={g.savebutton}  style={{ backgroundColor: '#f3d43c' }}>Save & Get code</button>
    ...
    <div className={g.cardcontainer}>
       ...
       { feeds.map(feed => ( 
        ...
        <p>{feed.description}</p>
            <Link href={feed.feedurl} alt={feed.title} target="_blank">Click here to view full article</Link>
        </div>
        ))
        }
        </div>

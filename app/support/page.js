...
export default function MainPage() {
  ...
  const [leftOffset, setLeftOffset] = useState(225);
  ...
  // Update left offset when sidebar state changes - this now properly updates during sticky mode
  useEffect(() => {
    const newLeftOffset = isCollapsed ? 70 : 225;
  ...
  }, [layoutHeight, isCollapsed]); // Added isCollapsed to dependencies
  ...
  return (
    <div className={p.pageWrapper} style={{ width: "100%" }}>
      ...
      <div ref={triggerRef} style={{ height: "1px", width: "50%" }} />

      {/* Placeholder to maintain document flow when sticky */}
      {enableInternalScroll && (
        <div style={{ height: `${layoutHeight}px`, width: "50%" }} />
      )}
      {/* Main content area */}
      <div
        ref={scrollableLayoutRef}
        className={`${p.scrollableLayout} ${
          enableInternalScroll ? p.stickyMode : p.normalMode
        }`}
        style={
          enableInternalScroll
            ? {
